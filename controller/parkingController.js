...
exports.getParkings = async (req, res) => {
  try {
     ...
     const parkings = await parkingRepository.findBySupervisor(userId);
...
exports.deleteParking = async (req, res) => {
  try {
    ...
    const deletedParking = await parkingRepository.delete(parkingId);
    if (!deletedParking) {
      return res.status(404).json({ error: 'Parking not found' });
    }
...
exports.getParkingById = async (req, res) => {
  try {
    const parking = await parkingRepository.findById(req.params.id);
    if (!parking) {
...
exports.updateParking = async (req, res) => {
  ...
  try {
    let parking = await parkingRepository.findById(req.params.id);
    if (!parking) {
      return res.status(404).json({ msg: 'Parking not found' });
    }
...
exports.getParkingsNearby = async (req, res) => {
  try {
    ...
    const parkings = await parkingRepository.findAll(); // Optimization needed: only select fields
...
exports.getParkingByName = async (req, res) => {
  try {
    ...
    const parkings = await parkingRepository.findByNameRegex(name);
    res.status(200).json(parkings);
...
exports.getParkingCount = async (req, res) => {
  try {
    const parkingCount = await parkingRepository.count();
...
exports.getParkingsBySupervisorId = async (req, res) => {
  try {
    ...
    const parkings = await parkingRepository.findBySupervisor(supervisorId);
    res.status(200).json(parkings);
...
exports.getParkingsByAdminEmail = async (req, res) => {
  try {
    ...
    const adminUser = await adminRepository.findByEmail(adminEmail);
    if (!adminUser) {
      return res.status(404).json({ error: 'Admin not found' });
    }

    // Find parkings associated with the admin
    const parkings = await parkingRepository.findByAdminEmail(adminUser.email);
