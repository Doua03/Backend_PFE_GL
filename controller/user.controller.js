...
exports.register = async (req, res, next) => {
  try {
    ...
    // ISP - Utilisation de AuthenticationService pour les opérations d'authentification
    ...
    // ISP - Utilisation de EmailService pour les opérations d'email
...
exports.login = async (req, res, next) => {
  try {
    ...
    // ISP - Utilisation de AuthenticationService pour les opérations d'authentification uniquement
    ...
    res
      .status(401)
      .json({ message: error.message || "Email or password is incorrect" });
  }
};
...
exports.delete = async (req, res) => {
  try {
    if (!ObjectId.isValid(req.params.userId))
      return res
        .status(400)
        .send(`No record with given id : ${req.params.userId}`);
    ...
    if (!deletedUser) {
      return res
        .status(404)
        .send(`User not found with ID: ${req.params.userId}`);
    }
  ...
  try {
    ...
    if (!expiryRegex.test(cardExpiry)) {
      return res.status(400).json({
        message: "Invalid card expiration date format. Please use MM/YY",
      });
    }

    console.log("Request body:", req.body);
    const result = await UserService.addPayment(
      userId,
      cardHolder,
      cardNumber,
      cardExpiry,
